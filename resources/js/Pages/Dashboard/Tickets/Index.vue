...
const props = defineProps({ tickets: { type: [Array, Object], required: true } });
...
const paginationLinks = computed(() => props.tickets?.links || []);
const from = computed(() => props.tickets?.from);
const to = computed(() => props.tickets?.to);
const total = computed(() => props.tickets?.total ?? ticketList.value.length);
...
<template>
    ...
    <AppLayout>
        ...
        <EmptyState v-if="ticketList.length === 0" title="تیکتی ثبت نشده است" description="برای طرح پرسش یا درخواست پشتیبانی، یک تیکت ایجاد کنید.">
            <template #action><Link :href="route('tickets.create')" class="ui-button ui-button-primary">ثبت تیکت</Link></template>
        </EmptyState>
        <template v-else>
            <div class="grid gap-3 md:hidden">
                <Link v-for="ticket in ticketList" :key="ticket.id" :href="route('tickets.show', ticket.id)" class="ui-card block p-4">
