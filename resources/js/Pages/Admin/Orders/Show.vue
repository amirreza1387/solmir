        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          ...
          <form @submit.prevent="updateStatus">
            ...
            <button
              ...
              class="w-full py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50 font-bold text-xs flex items-center justify-center gap-1.5 transition cursor-pointer shadow-xs"
            >
              <Loader2 v-if="form.processing" class="w-4 h-4 animate-spin ml-1" />
              <span>{{ form.processing ? 'در حال بروزرسانی...' : 'بروزرسانی وضعیت سفارش' }}</span>
            </button>
            <span v-if="form.recentlySuccessful" class="text-xs font-bold text-emerald-600 mt-2 block text-center">
